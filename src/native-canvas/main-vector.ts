export class FnApp {
  ...
  drawAxis() {
    this.ctx2d?.save();
    const {
      config: { startX, endX, startY, endY },
      arrowLen,
    } = this;
    // 绘制 x 轴
  ...
  drawAxisLabel() {
    ...
    const {
      gridSize,
      config: { fontSize, startX, endX, startY, endY },
    } = this;
    ...
    for (let i = 0; i * unitX < endX; i++) {
      ...
      ctx2d.fillText(
        label,
        i * gridSize.w - ctx2d.measureText(label).width / 2,
        fontSize
      );
    }
    // 绘制 x 负半轴刻度
    ...
    for (let i = 0; i * unitY < endY; i++) {
      ...
      ctx2d.fillText(
        label,
        -ctx2d.measureText(label).width,
        i * gridSize.h + fontSize / 2
      );
    }
    // 绘制 y 负半轴刻度
  ...
  public handleEvent(e: Event): void {
    switch (e.type) {
      ...
      case "mousemove":
        ...
        if (this.isMouseDown)
          this.dispatchMouseDrag(e as MouseEvent, this.getMousePos(e));
        break;
      case "mousewheel":
