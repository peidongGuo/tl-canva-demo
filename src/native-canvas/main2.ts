...
export class FnApp {
  ...
  public gridCount: number | undefined;
  public xGridCount: number;
  public yGridCount: number;
  ...
  constructor(canvas: HTMLCanvasElement, opts: IConfig = {}) {
    ...
    this.rightX = opts.rightX || -this.leftX;
    this.xLen = this.rightX - this.leftX;
    // 初始化时，y 的取值默认和 x 值一样，因为他们代表的值一样，不然 leftY，rightY 也要当做参数传进来
    this.leftY = opts.leftY || this.leftX;
    this.rightY = opts.rightY || this.rightX;
    this.yLen = this.rightY - this.leftY;

    // this.gridCount = opts.gridCount || 10;
    this.xGridCount = opts.xGridCount || 10;
    this.yGridCount = opts.yGridCount || 10;
  ...
  handleMouseWheel(e: Event) {
    ...
    let xScale: number, yScale;
    if (deltaY > 0) {
      // 缩小
      xScale = 1 + scaleSteps;
      yScale = 1 + scaleSteps * 2;
    } else {
      // 放大
      xScale = 1 - scaleSteps;
      yScale = 1 - scaleSteps * 2;
    }
    if (this.isInvalidVal(xScale) || this.isInvalidVal(yScale)) return;
    const { x, y } = this.canvasPosToFnVal(canvasPos);
    // 注意缩放和拖拽不一样，left 的左右两边一边是加一边是减
    const tempLeftX = x - (x - leftX) * xScale;
    const tempLeftY = y - (y - leftY) * yScale;
    if (tempLeftX < 0 || tempLeftY < 0) return;

    this.leftX = x - (x - leftX) * xScale;
    this.rightX = x + (rightX - x) * xScale;
    this.leftY = y - (y - leftY) * yScale;
    this.rightY = y + (rightY - y) * yScale;
    this.xLen = this.rightX - this.leftX;
    this.yLen = this.rightY - this.leftY;
    this.draw();
  }
  ...
  drawGrid() {
    ...
    const [gridWidth, gridHeight, xFixedCount, yFixedCount] = this.calcGridSize(
      xLen,
      this.xGridCount,
      yLen,
      this.yGridCount
    ...
    for (; i * gridWidth < rightX; i++) {
      ...
      this.fillText(
        String(this.formatNum(i * gridWidth, xFixedCount)),
        x,
        height,
        this.fontSize,
        TextAlign.Center
    ...
    for (; j * gridHeight < rightY; j++) {
      ...
      this.fillText(
        String(this.formatNum(j * gridHeight, yFixedCount)),
        0,
        y,
        this.fontSize,
        TextAlign.Middle
  ...
  calcGridSize(
    ...
    let unitX = xLen / xGridCount;
    while (xGridWidth < unitX) {
      xGridWidth *= 10;
    }
    while (xGridWidth / 10 > unitX) {
      xGridWidth /= 10;
      xFixedCount++;
    }
    if (xGridWidth / 5 > unitX) {
      xGridWidth /= 5;
      xFixedCount++;
    } else if (xGridWidth / 2 > unitX) {
      xGridWidth /= 2;
      xFixedCount++;
    }
    let yGridWidth = 1;
    let yFixedCount = 0;
    let unitY = yLen / yGridCount;
    while (yGridWidth < unitY) {
      yGridWidth *= 10;
    }
    while (yGridWidth / 10 > unitY) {
      yGridWidth /= 10;
      yFixedCount++;
    }
    if (yGridWidth / 5 > unitY) {
      yGridWidth /= 5;
      yFixedCount++;
    } else if (yGridWidth / 2 > unitY) {
      yGridWidth /= 2;
      yFixedCount++;
    }
    return [parseInt(unitX + ""), parseInt(unitY + ""), 0, 0];
    // 因为 x 轴长度和 y 轴的长度是一样的，所以可以这样赋值
    return [xGridWidth, yGridWidth, xFixedCount, yFixedCount];
  }
  /** 绘制函数曲线，就是用一段段直线连起来 */
  drawFn() {
  ...
  handleCrosspoint(x: number) {
    ...
    pointList.forEach((point) => {
      ...
      this.fillText(
        `[${this.formatNum(point.x, this.fixedCount + 1)}, ${this.formatNum(
          point.y,
          this.fixedCount + 1
        )}]`,
        x,
